...
//  Copyright © 2017 John Talbot BeNiceGames.com. All rights reserved.
//

// This program is a lexicon data structure known as the "Directed Acyclic Word Graph", more commonly
// used as a quick way to UnScramble Letters.

// A lot of effort was made to not only create a DAWG from a list of words, but to understand how to
// reverse the process and recreate the list from the DAWG itself.

// To set up the DAWG, all that is required is a sorted list of words.  If the maximum
// word length exceeds 50 characters, please change INPUT_LIMIT. That is all.

// The unsigned integer encoding that we are using for this "DAWG" allows for up to 33554432 nodes.
// That is more than most languages have, including English.
...
// The UniqueID is used for nothing but debugging. It is easier to tell what node your working with if each node has a different identifier.
...
// Count the "Tnode"'s in "ThisTrie" for each "MaxChildDepth".  "Count" needs to be an integer array of size "MAX_WORD_LENGTH".
...
// By defining CLANG_ANALYZER_NORETURN, the analyzer wont complain about allocatig memory, without dealocating it in the same function
...
dawgPtr DawgFromRAMDictionary(char *Dictionary[], int *WordsOfLength)
{
	...
	// Start at the largest "WORD LENGTH" and work down from there for recursive reduction to take place early on to reduce the work load for the shallow nodes.
...
Bool getMAX_WORD_LENGTH_AND_COUNT(FILE *dict)
{
    ...
    // A Trie must know the MAX WORD LENGTH before it starts so that memory is not constantly being resized and take forever for large dictionaries to create a DAWG.
    
    // This will get the largest word size, compare line count in file with actual line count. Finally it positions the FILE handler at the first dictionary word to be read.
        ...
        lineCountInFile != NUMBER_OF_LINES_IN_DICTIONARY)
    {
        ...
        // Even though it does not match, it is a number, so position the file handler past it, so the dictionary can be read.
        rewind(dict);
        fgets(thisLine, INPUT_LIMIT, dict);
...
dawgPtr dawgFromWordFile(FILE *dict)
{
    ...
    // This will get thelargest word size, compare line count in file with actuall line count. Finally it positions the FILE handler at tge first dictionary  word to read.
    ...
    for ( int X = 0; X <= MAX_WORD_LENGTH; X++ )
    {
        if ( numberOfWordsOfLength[X] != numberOfWordsOfLength[X] )
        {
...
char ** dictionaryFromBinaryFile(FILE *in, unsigned int  *node_count, int **numberOfWordsOfLength, FILE *out)
{
    ...
    // Hey, Why not convert the TNodes to a dawg you say?.  Well you can't because this HolderOfAllTnodes is a one dimensional array while the dawg has a two dimensional array. Even if you extract the node_number_count_for_wordlengths to try and construct the two dimensional array, you still cannot because the order of the Tnodes do not represent the original order and size.
    // So by converting it to a word list and then to a dawg, we know that works as it is supposed to.
...
int main(int argc, char* argv[])
{
    ...
    if (argc <=1 )
    {
        ...
        int last = -1;
        int c;
        while ((c=getopt(argc, argv, ":hc:d:e:g:X:o:")) != -1)
        {
            switch (c) {
                case 'h': // help
                {
